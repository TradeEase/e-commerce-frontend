...
const ShoppingCart = () => {
  ...
  useEffect(() => {
    const fetchCartData = async () => {
      try {
        ...
        const cartResponse = await axios.get(`http://gateway:8080/api/orders/carts/user/${userId}`);
        console.log('Cart Response:', cartResponse.data);

        const cartId = cartResponse.data.cartId;

        const cartItemsResponse = await axios.get(`http://gateway:8080/api/orders/cartItems/cart/${cartId}`);
        console.log('Cart Items Response:', cartItemsResponse.data);

        const cartItems = cartItemsResponse.data;

        const productDetailsPromises = cartItems.map(item =>
          axios.get(`http://gateway:8080/api/product/products/${item.productId}`)
        );
  ...
  const handleQuantityChange = async (id, amount) => {
    ...
    if (updatedProduct) {
      try {
        ...
        if (amount !== 0) {
          await axios.put(`http://gateway:8080/api/orders/cartItems/${updatedProduct.cartItemId}`, {
            cartItemId: updatedProduct.cartItemId,
            productId: updatedProduct.productId,
            quantity: updatedProduct.quantity,
            cart: updatedProduct.cartId,
  ...
  const handleRemove = async (id) => {
    ...
    if (confirmRemove) {
      ...
      if (productToRemove) {
        try {
          await axios.delete(`http://gateway:8080/api/orders/cartItems/${productToRemove.cartItemId}`);
  ...
  const handleCheckout = async () => {
    try {
      ...
      for (const item of cart) {
        ...
        await axios.post('http://gateway:8080/api/orders', orderPayload);
      }
      ...
      const deleteCartItemPromises = cart.map(item => {
        ...
        return axios.delete(`http://gateway:8080/api/orders/cartItems/${item.cartItemId}`);
      });
